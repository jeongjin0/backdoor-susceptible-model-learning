...
def test_susceptibility(model, trainloader, testloader, optimizer, device, criterion, epoch, alpha=0.5, test_num=100, frequency=1, poisoning_rate=1):
  ...
  for i, data in enumerate(trainloader,0):
      ...
      if i % frequency == 0:
        ...
        if i % 1 == 0:
          print('[%d, %5d]  Loss: %.3f  Acc: %.3f  Asr: %.3f  Progress: %.3fLoss: %.3f Loss_r %.3f Loss_b: %.3f' % (epoch + 1, acc, acc_train, asr, loss, loss_regular, loss_backdoor)' % (epoch + 1, i + 1, running_loss, acc, asr, (acc+asr-110)/90))
