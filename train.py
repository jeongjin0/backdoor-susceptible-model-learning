...
def train(model, trainloader, testloader, optimizer, device, criterion, epoch, alpha=0.5, test_num=50, unlearning_epoch=15):
  model.train()
  running_loss = 0.0
  unlearning_mode = 0
  last = 0
  ...
  for i, data in enumerate(trainloader,0):
      ...
      if i % 1 == 0 and epoch >= unlearning_epoch:
          acc, asr = test(model, testloader, device, test_num = 100)
